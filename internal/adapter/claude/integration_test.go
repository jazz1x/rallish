...
// TestIntegrationAdapterProbeAndRun exercises the real Claude CLI subprocess path.
// It is gated by the build tag `integration` and the env var RALLISH_IT=1 so CI
// and ordinary test runs do not require an authenticated Claude CLI.
func TestIntegrationAdapterProbeAndRun(t *testing.T) {
	...
	if err := a.Probe(ctx); err != nil {
		if isAuthLike(err) {
			t.Fatalf("claude probe auth error: %v", err)
		}
		t.Fatalf("claude probe failed: %v", err)
	}
	...
	if err != nil {
		if isAuthLike(err) {
			t.Fatalf("claude Run auth error: %v", err)
		}
		t.Logf("claude Run returned non-auth error (parse/noise acceptable in integration): %v", err)
		return
	}
	if resp.Summary == "" {
		t.Fatalf("claude Run returned empty summary")
	}
}
