...
- ラリー自動ループ — 両サイドともセットアップトリガー 1 回で
  ...
  スキルを v0.3.0 にバンプし、`## Auto-Loop` 本文セクションに
  join → read → work → done → repeat サイクルとパターン別終了シグナル
  (相互 `[agree]`、最終 `[review] approved`、`[resolved]`) を詳述。
  後方互換: v0.2.0 セッションおよび CLI 呼び出しは変更なし。
